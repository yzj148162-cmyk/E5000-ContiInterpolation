...
typedef struct {
    ...
    float rollVel, pitchVel, yawVel;		// angular velocity
    float rollAvel, pitchAvel, yawAvel;		// angular acceleration
    float xVel, yVel, zVel, rVel;			// velocity
    float xAvel, yAvel, zAvel, rAvel;		// acceleration
    int reserve[4];							// retention parameter
} sRigidBodyExtendData;

//扩展数据类型
typedef struct {
    int type;									// 扩展数据类型
    int number;									// 实际数量
    int length;									// 扩展数据占用字节长度
    union {
        sRigidBodyExtendData rigidBodyExtend[MAX_RIGIDBODIES];	//刚体扩展数据
        //骨骼扩展数据
        //markerset扩展数据
        //模拟通道扩展数据
        //marker扩展数据
...
} sExtendData, * psExtendData;

//
typedef struct {
    int nExtendDataNum;								//扩展数据数量
    sExtendData extendData[ExtendDataEndType];		//扩展数据
} sFrameExtendData, * psFrameExtendData;
...
typedef struct
{
    ...
    short params;                               // host defined parameters
    sFrameExtendData extendData;				// 扩展数据
} sFrameOfMocapData;
