...
输入、离线PVT位控、控制状态和8绳监视五个子页。离线PVT首版已接通
“末端五次轨迹—逆运动学—虚拟绞盘—8轴PVT装表与同步启动”链路。
Nokov几何位姿重建、真实F/T Trace对象、8轴编码器启动基准和实时力交互实机链
尚未接通，因此通用“开始控制”入口仍保持禁用。

下一阶段先以8台空载电机验证两条互斥执行链：

- 模拟六维力、与UI一致的Newmark控制周期、8个独立PID和虚拟绞盘映射组成
  的实时速度闭环；当前已知五次轨迹测试使用独立主机参考轨迹缓存，逐周期调用
