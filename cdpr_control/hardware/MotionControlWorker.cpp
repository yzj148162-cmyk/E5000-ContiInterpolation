    ...
    , cdprAnalysisWatcher_(new QFutureWatcher<CdprVirtualConsistencyAnalysisResult>(this))
{
    ...
    connect(feedbackTimer_, &QTimer::timeout, this, &MotionControlWorker::refreshFeedback);
    connect(velocityControlTimer_, &QTimer::timeout,
            this, &MotionControlWorker::runVelocityControlCycle);
...
void MotionControlWorker::initializeBoard(const ContiTestConfig &config)
{
...
void MotionControlWorker::closeBoard()
{
    shutdownHardware();
}

void MotionControlWorker::refreshBusCycle()
{
    if (!boardInitialized_) {
...
void MotionControlWorker::startTelemetryRecording()
{
    if (!boardInitialized_) {
...
void MotionControlWorker::stopTelemetryRecording()
{
...
void MotionControlWorker::shutdownHardware()
{
...
void MotionControlWorker::enableSelectedAxes(const ContiTestConfig &config)
{
    if (!boardInitialized_) {
...
void MotionControlWorker::disableSelectedAxes(const ContiTestConfig &config)
{
    if (!boardInitialized_) {
        return;
    }
...
void MotionControlWorker::enableAllDetectedAxes()
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::disableAllDetectedAxes()
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::startTest(const ContiTestConfig &config)
{
    if (!boardInitialized_) {
...
void MotionControlWorker::enableJogAxis(const SingleAxisJogConfig &config)
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::disableJogAxis(const SingleAxisJogConfig &config)
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::setJogAxisZero(const SingleAxisJogConfig &config)
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::startPointMove(const SingleAxisJogConfig &config)
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::stopPointMove(bool emergency)
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
bool MotionControlWorker::validateVelocityControlConfig(
...
void MotionControlWorker::startVelocityControl(const VelocityControlConfig &requestedConfig)
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::evaluateVelocityReference(double elapsedS,
...
void MotionControlWorker::runVelocityControlCycle()
{
    if (!velocityControlActive_) {
        return;
    }
...
void MotionControlWorker::finishVelocityControl(const QString &message, bool emergency)
{
    velocityControlTimer_->stop();
...
void MotionControlWorker::appendVelocityPlotFrames(const QVector<TraceTelemetryFrame> &frames)
{
    velocityBatchAlignedTrackingErrorValid_ = false;
    velocityBatchPeakAlignedTrackingErrorDegree_ = 0.0;
...
void MotionControlWorker::flushVelocityPlotSamples()
{
    if (pendingVelocityPlotSamples_.isEmpty()) {
        return;
    }
...
void MotionControlWorker::stopVelocityControl(bool emergency)
{
    if (!velocityControlActive_) {
...
void MotionControlWorker::resetVelocityController()
{
    if (velocityControlActive_) {
...
bool MotionControlWorker::validateTorqueTestConfig(
...
int MotionControlWorker::torquePercentToRaw(double torquePercent) const
{
...
void MotionControlWorker::writeTorqueVelocityLimit(const TorqueTestConfig &config)
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::startTorqueTest(const TorqueTestConfig &requestedConfig)
{
    if (!boardInitialized_) {
        emit logMessage(QStringLiteral("请先初始化控制卡。"));
        return;
...
void MotionControlWorker::updateTorqueCommand(const TorqueTestConfig &config)
{
...
void MotionControlWorker::runTorqueTestCycle()
{
    if (!torqueTestActive_) {
        return;
    }
...
void MotionControlWorker::flushTorquePlotSamples()
{
...
void MotionControlWorker::finishTorqueTest(const QString &message, bool emergency)
{
    torqueTestTimer_->stop();
    flushTorquePlotSamples();
...
void MotionControlWorker::stopTorqueTest(bool emergency)
{
    if (!torqueTestActive_) {
...
bool MotionControlWorker::validateTraceDelayCalibrationConfig(
...
void MotionControlWorker::startTraceDelayCalibration(
...
bool MotionControlWorker::startNextTraceDelaySegment(QString &errorMessage)
{
...
void MotionControlWorker::runTraceDelayCalibrationCycle()
{
    if (!traceDelayCalibrationActive_) {
        return;
    }
...
void MotionControlWorker::stopTraceDelayCalibration(bool emergency)
{
    if (!traceDelayCalibrationActive_) {
...
void MotionControlWorker::finishTraceDelayCalibration(const QString &message,
                                               bool failed,
                                               bool emergency)
{
    traceDelayCalibrationTimer_->stop();
...
void MotionControlWorker::resetTraceDelayCalibrationAxis(quint16 axis)
{
    if (traceDelayCalibrationActive_) {
...
void MotionControlWorker::appendTraceDelayCalibrationFrames(
...
void MotionControlWorker::resetTraceDelayHistory()
{
...
QString MotionControlWorker::traceDelayCalibrationFilePath() const
{
    const QString directory =
...
void MotionControlWorker::loadTraceDelayCalibrationResults()
{
...
void MotionControlWorker::saveTraceDelayCalibrationResults()
{
...
bool MotionControlWorker::waitForAxisStop(quint16 axis, int timeoutMs) const
{
...
bool MotionControlWorker::waitForContiStop(int timeoutMs) const
{
    const int attempts = std::max(1, timeoutMs / kStopPollIntervalMs);
    for (int attempt = 0; attempt < attempts; ++attempt) {
...
bool MotionControlWorker::safelyStopPointAxis(quint16 axis, const QString &reason)
{
    QString error;
...
void MotionControlWorker::safelyStopAllMotionForShutdown()
{
    producerTimer_->stop();
    monitorTimer_->stop();
    velocityControlTimer_->stop();
...
void MotionControlWorker::disableAllEnabledAxesForShutdown()
{
...
void MotionControlWorker::produceNextPoint()
{
...
void MotionControlWorker::enqueueTrajectoryPoint(const ContiPoint &point)
{
...
void MotionControlWorker::submitGeneratedPoints()
{
...
bool MotionControlWorker::hasExecutionDelayReady() const
{
...
bool MotionControlWorker::generateAllTrajectoryPoints()
{
...
bool MotionControlWorker::startAfterPreload()
{
    QString error;
...
double MotionControlWorker::planTimeForMark(long currentMark) const
{
...
double MotionControlWorker::referenceVectorSpeed(double planTimeS) const
{
    if (pushedPointsByMark_.isEmpty()) {
        return 0.0;
    }
...
double MotionControlWorker::calculateRatioCommand(long currentMark, double bufferTimeS, qint64 elapsedMs)
{
...
bool MotionControlWorker::applyRatioCommand(qint64 elapsedMs, QString &errorMessage)
{
...
void MotionControlWorker::monitorContinuousRun()
{
    if (!running_) {
        return;
    }
...
bool MotionControlWorker::traceReachedFinalTarget() const
{
    if (pushedPointsByMark_.isEmpty()) {
        return false;
    }
...
void MotionControlWorker::finishRun(const QString &message)
{
...
void MotionControlWorker::enterError(const QString &message)
{
...
void MotionControlWorker::resetRunTimingState()
{
...
void MotionControlWorker::publishStatus()
{
    ContiStatus status;
...
bool MotionControlWorker::configureBaseAxes(const ContiTestConfig &config)
{
    if (detectedAxes_.isEmpty()) {
...
void MotionControlWorker::refreshAxisStates()
{
    for (const quint16 axis : traceAxes_) {
...
bool MotionControlWorker::bothAxesEnabled(const ContiTestConfig &config) const
{
