...
public:
    ...
    {
        if (!card_.clearAxisFaults(cardNo_, axis, notice, error)
...
MotionCardHardwareInterface::MotionCardHardwareInterface()
...
bool MotionCardHardwareInterface::initializeBoard(quint16 cardNo, short &boardCount, QString &error)
{ return invokeHardware(backend_, [&] { return backend_->initialize(cardNo, boardCount, error); }); }
bool MotionCardHardwareInterface::closeBoard(QString &error)
{ return invokeHardware(backend_, [&] { return backend_->close(error); }); }
bool MotionCardHardwareInterface::setBusCycle(int cycleUs, QString &error)
...
bool MotionCardHardwareInterface::disableAxis(quint16 axis, QString &error)
{ return invokeHardware(backend_, [&] { return backend_->card_.disableAxis(backend_->cardNo_, axis, error); }); }
bool MotionCardHardwareInterface::readCommandPosition(quint16 axis, double &position, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.readPositionUnit(backend_->cardNo_, axis, position, error); }); }
bool MotionCardHardwareInterface::readTargetPositionUnit(quint16 axis, double &position, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.readTargetPositionUnit(backend_->cardNo_, axis, position, error); }); }
bool MotionCardHardwareInterface::startPointMove(const SingleAxisJogConfig &config, QString &error)
{
    return invokeHardware(backend_, [&] {
...
bool MotionCardHardwareInterface::startVelocityMove(
...
bool MotionCardHardwareInterface::changeVelocity(
    const VelocityControlConfig &config,
...
bool MotionCardHardwareInterface::changeTorque(
...
bool MotionCardHardwareInterface::readTorque(
...
bool MotionCardHardwareInterface::writeTorqueVelocityLimit(
...
bool MotionCardHardwareInterface::readTorqueDriveDiagnostic(
...
bool MotionCardHardwareInterface::stopAxis(quint16 axis, bool emergency, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.stopAxis(backend_->cardNo_, axis, emergency, error); }); }
bool MotionCardHardwareInterface::stopAxis(quint16, quint16 axis, bool emergency, QString &error) const
{ return stopAxis(axis, emergency, error); }
bool MotionCardHardwareInterface::axisMotionDone(quint16 axis, bool &done, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.axisMotionDone(backend_->cardNo_, axis, done, error); }); }
bool MotionCardHardwareInterface::axisRunMode(quint16 axis, quint16 &mode, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.axisRunMode(backend_->cardNo_, axis, mode, error); }); }
bool MotionCardHardwareInterface::configureAndOpen(const ContiTestConfig &config, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.configureAndOpen(config, error); }); }
bool MotionCardHardwareInterface::pushLine(const ContiTestConfig &config, const ContiPoint &point, long mark, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.pushLine(config, point, mark, error); }); }
bool MotionCardHardwareInterface::start(const ContiTestConfig &config, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.start(config, error); }); }
bool MotionCardHardwareInterface::startStreaming(const ContiTestConfig &config,
                                            const QVector<ContiFeedItem> &points,
                                            bool preloadAllToCard,
                                            QString &error) const
{
...
void MotionCardHardwareInterface::appendStreamingPoints(const QVector<ContiFeedItem> &points) const
{
    if (points.isEmpty()) {
        return;
    }
...
ContiFeedStatus MotionCardHardwareInterface::streamingStatus() const
{ return invokeHardware(backend_, [&] { return backend_->streamingStatus(); }); }
ContiSpeedRatioResult MotionCardHardwareInterface::changeSpeedRatio(const ContiTestConfig &config,
                                                                QString &error) const
{
    return invokeHardware(backend_, [&] {
...
bool MotionCardHardwareInterface::stop(const ContiTestConfig &config, bool emergency, QString &error) const
{
    return invokeHardware(backend_, [&] {
...
bool MotionCardHardwareInterface::closeList(const ContiTestConfig &config, QString &error) const
{
    return invokeHardware(backend_, [&] {
...
bool MotionCardHardwareInterface::contiMotionDone(const ContiTestConfig &config) const
{ return invokeHardware(backend_, [&] { return backend_->card_.isContiMotionDone(config); }); }
bool MotionCardHardwareInterface::readContiCardReadback(const ContiTestConfig &config,
                                                    ContiCardReadback &readback,
                                                    QString &error) const
{
    return invokeHardware(backend_, [&] {
        return backend_->card_.readContiCardReadback(config, readback, error);
    });
}
bool MotionCardHardwareInterface::readVectorSpeedDegreePerSecond(
    const ContiTestConfig &config,
    double &speedDegreePerSecond,
    QString &error) const
{
    return invokeHardware(backend_, [&] {
        return backend_->card_.readVectorSpeedDegreePerSecond(
            config, speedDegreePerSecond, error);
    });
}
long MotionCardHardwareInterface::currentMark(const ContiTestConfig &config) const
{ return invokeHardware(backend_, [&] { return backend_->card_.currentMark(config); }); }
long MotionCardHardwareInterface::remainSpace(const ContiTestConfig &config) const
{ return invokeHardware(backend_, [&] { return backend_->card_.remainSpace(config); }); }
short MotionCardHardwareInterface::runState(const ContiTestConfig &config) const
...
bool MotionCardHardwareInterface::traceConfigured() const
{ return invokeHardware(backend_, [&] { return backend_->feedbackTrace_.isConfigured(); }); }
bool MotionCardHardwareInterface::traceEverRead() const
{ return invokeHardware(backend_, [&] { return backend_->feedbackTrace_.hasEverRead(); }); }
int MotionCardHardwareInterface::traceFramesRead() const
{ return invokeHardware(backend_, [&] { return backend_->traceFramesRead_; }); }
int MotionCardHardwareInterface::traceSamplePeriodUs() const
...
short MotionCardHardwareInterface::traceLastApiResult() const
{ return invokeHardware(backend_, [&] { return backend_->feedbackTrace_.lastApiResult(); }); }
QString MotionCardHardwareInterface::traceStateText() const
{ return invokeHardware(backend_, [&] { return backend_->traceStateText_; }); }
quint16 MotionCardHardwareInterface::busErrorCode() const
{ return invokeHardware(backend_, [&] { WORD code = 0; nmc_get_errcode(backend_->cardNo_, 2, &code); return static_cast<quint16>(code); }); }
bool MotionCardHardwareInterface::setAxisEquivalent(quint16, quint16 axis, double pulsePerUnit, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.setAxisEquivalent(backend_->cardNo_, axis, pulsePerUnit, error); }); }
bool MotionCardHardwareInterface::clearAxisFaults(quint16, quint16 axis, QString &notice, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.clearAxisFaults(backend_->cardNo_, axis, notice, error); }); }
bool MotionCardHardwareInterface::enableAxis(quint16, quint16 axis, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.enableAxis(backend_->cardNo_, axis, error); }); }
bool MotionCardHardwareInterface::disableAxis(quint16, quint16 axis, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.disableAxis(backend_->cardNo_, axis, error); }); }
bool MotionCardHardwareInterface::readPositionUnit(quint16, quint16 axis, double &position, QString &error) const
{ return invokeHardware(backend_, [&] { return backend_->card_.readPositionUnit(backend_->cardNo_, axis, position, error); }); }
bool MotionCardHardwareInterface::isAxisMotionDone(quint16, quint16 axis) const
{
    return invokeHardware(backend_, [&] {
        bool done = false;
        QString ignored;
...
bool MotionCardHardwareInterface::isContiMotionDone(const ContiTestConfig &config) const
