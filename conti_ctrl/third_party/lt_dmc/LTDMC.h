...
DMC_API short __stdcall dmc_board_init_eth(WORD CardNo,const char* IpAddr);
//减速停止距离
...
//轨迹算法升级连续插补新增功能：包括减速角停止角、拐弯时间、小圆限速、衔接点速度规划模式设置等功能
...
//总线卡二维螺距补偿功能
DMC_API short __stdcall dmc_set_leadscrew_comp_2D_config_unit_ex(WORD CardNo, WORD table_index, WORD axis, WORD* ref_axis, double* ref_axis_start_pos, double* ref_axis_length, WORD* ref_axis_segment, double* value);
DMC_API short __stdcall dmc_get_leadscrew_comp_2D_config_unit_ex(WORD CardNo, WORD table_index, WORD* axis, WORD* ref_axis, double* ref_axis_start_pos, double* ref_axis_length, WORD* ref_axis_segment, double* value);
...
DMC_API short __stdcall dmc_get_leadscrew_comp_2D_angle_unit_ex(WORD CardNo, WORD* table_index, WORD* axis, WORD* ref_axis, double* ref_axis_start_pos, double* ref_axis_length, double *angle);
...
//5X10系列连续插补中相对于轨迹段终点 DA 提前/滞后跟随输出
...
//环网冗余相关状态
DMC_API short __stdcall nmc_get_cable_redundancy_enable(WORD CardNo, WORD* Enable, WORD* ErrStop);
DMC_API short __stdcall nmc_get_num_connected_slaves_red(WORD CardNo, uint32* BrkMainSlaves, uint32* BrkRedSlaves, uint32* CurMainSlaves, uint32* CurRedSlaves);

DMC_API short __stdcall get_bar2_value(WORD CardNo, WORD offset , WORD length, uint32* value);

//获取编码器速度功能
DMC_API short __stdcall dmc_get_encoder_speed(WORD CardNo, WORD axis, double *vel);

typedef struct{
	 float fl_val;//pwm随动值（因变量）
     float ctrl_val;//控制值（自变量）
} PwmCurve_CtrlPoint;

typedef struct{
	 float vol_val;//da值（因变量）
     float ctrl_val;//控制值（自变量）
} DaCurve_CtrlPoint;

//5X10系列连续插补DA--T/P跟随输出功能
DMC_API short __stdcall dmc_set_da_curve(WORD CardNo, WORD Crd, WORD curve_no, WORD ctrl_mode,WORD ctrl_point_num, DaCurve_CtrlPoint * ctrl_point);
DMC_API short __stdcall dmc_get_da_curve(WORD CardNo, WORD Crd, WORD curve_no, WORD* ctrl_mode,WORD* ctrl_point_num, DaCurve_CtrlPoint * ctrl_point);
DMC_API short __stdcall dmc_conti_da_curve_control_to_start(WORD CardNo, WORD Crd, WORD curve_no, WORD da_channel,WORD delay_mode,float delay_value,uint32 mark);
DMC_API short __stdcall dmc_conti_da_curve_control_to_stop(WORD CardNo, WORD Crd, WORD curve_no, WORD da_channel, WORD ahead_mode,float ahead_value,uint32 mark);
DMC_API short __stdcall dmc_conti_da_curve_control(WORD CardNo, WORD Crd, WORD curve_no, WORD on_off,WORD da_channel,uint32 us_delay_time,uint32 mark);

DMC_API short __stdcall dmc_cmd_buf_set_axis_profile_ext(WORD CardNo, WORD group, WORD axis_num, WORD* axis_list, WORD* plan_mode, double* start_vel, double* max_vel, double* stop_vel, double* tacc, double* tdec, double* ja, double* jd);

DMC_API short __stdcall dmc_set_node_da_output (WORD CardNo,WORD PortNum,WORD NodeNum, WORD Group,WORD* Target_Time,WORD* Target_Voltage);
DMC_API short __stdcall dmc_set_node_DA_configure(WORD CardNo,WORD PortNum,WORD NodeNum, WORD Channel,WORD Group);
DMC_API short __stdcall dmc_set_node_DA_enable(WORD CardNo,WORD PortNum,WORD NodeNum, WORD Channel,WORD DelayPeriod);
DMC_API short __stdcall dmc_set_node_DA_stop(WORD CardNo,WORD PortNum,WORD NodeNum, WORD Channel);
DMC_API short __stdcall dmc_set_node_DA_mode(WORD CardNo,WORD Mode, WORD Address,WORD DataLen,WORD Value, WORD DelayPeriod);


DMC_API short __stdcall dmc_set_acc_jerkratio_profile_unit (WORD CardNo, WORD axis, double Min_Vel, double Max_Vel, double averageAcc, double averageDec, double jerkAccRatio, double jerkDecRatio ,double stop_vel);
DMC_API short __stdcall dmc_get_acc_jerkratio_profile_unit (WORD CardNo, WORD axis, double* Min_Vel,  double* Max_Vel,double* averageAcc,double* averageDec,  double * jerkAccRatio,  double*  jerkDecRatio ,  double * stop_vel);
DMC_API short __stdcall dmc_set_acctime_jerkratio_profile_unit (WORD CardNo, WORD axis, double Min_Vel, double Max_Vel, double accTime, double decTime, double jerkAccRatio, double jerkDecRatio ,double stop_vel);
DMC_API short __stdcall dmc_get_acctime_jerkratio_profile_unit (WORD CardNo, WORD axis, double* Min_Vel,  double* Max_Vel,double* accTime,double* dectime,  double * jerkAccRatio,  double* jerkDecRatio ,double * stop_vel);
DMC_API short __stdcall dmc_set_vector_acc_jerkratio_profile_unit (WORD CardNo, WORD CrdID, double Min_Vel, double Max_Vel, double averageAcc, double averageDec, double jerkAccRatio, double jerkDecRatio ,double stop_vel);
DMC_API short __stdcall dmc_get_vector_acc_jerkratio_profile_unit (WORD CardNo, WORD CrdID, double* Min_Vel,  double* Max_Vel,double* averageAcc,double* averageDec,  double * jerkAccRatio,  double* jerkDecRatio ,double * stop_vel);
DMC_API short __stdcall dmc_set_vector_acctime_jerkratio_profile_unit (WORD CardNo, WORD axis, double Min_Vel, double Max_Vel, double accTime, double decTime, double jerkAccRatio, double jerkDecRatio ,double stop_vel);
DMC_API short __stdcall dmc_get_vector_acctime_jerkratio_profile_unit (WORD CardNo, WORD CrdID, double* Min_Vel,double* Max_Vel,double* accTime,double* decTime,  double * jerkAccRatio,  double* jerkDecRatio ,double * stop_vel);

DMC_API short __stdcall  dmc_measure_time_enable(WORD CardNo,WORD enable);

DMC_API short __stdcall dmc_m_cmd_buf_set_profile_extern(WORD CardNo, WORD group, WORD axis_num, WORD* axis_list, WORD* plan_mode, double* start_vel, double* max_vel, double* stop_vel, double* tacc, double* tdec, double* ja, double* jd);

//脉冲卡回零功能扩展：回零原点和EZ信号支持设置寻找距离
DMC_API short __stdcall dmc_set_home_search_limit(WORD CardNo, WORD axis, WORD enable, WORD stop_mode, uint32 limit_length, uint32 org_length,uint32 ez_length);
DMC_API short __stdcall dmc_set_home_search_limit_unit(WORD CardNo, WORD axis, WORD enable, WORD stop_mode, double limit_length,double org_length, double ez_length);

DMC_API short __stdcall dmc_get_traj_algorithm_version(WORD CardNo,char *trajAlgorithmVersion);

//总线卡适配ISMC驱动器软着陆定制功能
DMC_API short __stdcall dmc_set_pt_soft_land_profile(WORD CardNo, WORD AxisNo, int32 PpModePos, int32 ChangePos, uint32 PosTorLimit, uint32 DelayTime, uint32 KeepTorque, uint32 Mode);
DMC_API short __stdcall dmc_set_pt_soft_land_init_profile(WORD CardNo, WORD AxisNo, uint32 PpModeVel, uint32 PpModeAcc, uint32 PpModeDec, int32 SuccessMaxVel, int32 ReturnPos, int32 PvModeVel, uint32 PvModeSecondVel);
DMC_API short __stdcall dmc_get_pt_soft_land_arrive_flag(WORD CardNo, WORD AxisNo, WORD *Flag);
DMC_API short __stdcall dmc_start_pt_soft_land(WORD CardNo, WORD AxisNo, WORD RunMode);
DMC_API short __stdcall dmc_pt_soft_land_return(WORD CardNo, WORD AxisNo, WORD RunMode);
DMC_API short __stdcall dmc_stop_pt_soft_land(WORD CardNo, WORD AxisNo, WORD RunMode);
DMC_API short __stdcall dmc_clear_pt_soft_land_errcode(WORD CardNo, WORD AxisNo, WORD RunMode);
DMC_API short __stdcall dmc_set_pt_soft_land_PosTorLimit(WORD CardNo, WORD AxisNo, uint32 PosTorLimit);

DMC_API short __stdcall dmc_set_traj_min_travel_time(WORD CardNo, WORD Crd, double min_travel_time);
DMC_API short __stdcall dmc_get_traj_min_travel_time(WORD CardNo, WORD Crd, double* min_travel_time);

DMC_API short __stdcall dmc_conti_set_follow_params(WORD CardNo,WORD Crd,WORD Follow_type, WORD Master, WORD Slave);
DMC_API short __stdcall dmc_conti_get_follow_params(WORD CardNo,WORD Crd,WORD* Follow_type, WORD* Master, WORD* Slave);

DMC_API short __stdcall nmc_get_errcode_ex(WORD CardNo, WORD Channel, uint32 *ErrType, uint32 *ErrCode);
DMC_API short __stdcall nmc_fieldbus_state_get_record_info(WORD CardNo, WORD Channel, uint32 ReadErrNum, uint32 ReadErrIndex, uint32* TotalErrNum, WORD* ReturnErrNum, uint64 *TimeStamp, uint32* ErrIndex, WORD* SlaveAddr, uint32* ErrType, uint32* ErrCode, uint32* Info_0, uint32* Info_1, uint32* Info_2, uint32* Info_3);
DMC_API short __stdcall nmc_fieldbus_state_set_record_stop(WORD CardNo, WORD Stop);
DMC_API short __stdcall nmc_fieldbus_state_get_record_stop(WORD CardNo, WORD* Stop);
DMC_API short __stdcall nmc_fieldbus_state_set_record_mode(WORD CardNo, WORD Mode);
DMC_API short __stdcall nmc_fieldbus_state_get_record_mode(WORD CardNo, WORD* Mode);
DMC_API short __stdcall nmc_set_system_time(WORD CardNo, WORD Year, WORD Mon, WORD Day, WORD Hour, WORD Min, WORD Sec);
DMC_API short __stdcall nmc_get_system_time(WORD CardNo, WORD* Year, WORD* Mon, WORD* Day, WORD* Hour, WORD* Min, WORD* Sec);
DMC_API short __stdcall nmc_set_master_state(WORD CardNo, uint32 States);
DMC_API short __stdcall nmc_get_master_diagnosis_info(WORD CardNo, uint32* NumSlavesFound,  uint32* NumDCSlavesFound,  uint32* NumCfgSlaves, uint32* NumMbxSlaves, uint32* TXFrames, uint32* RXFrames, uint32* LostFrames, uint32 * CyclicFrames, uint32* CyclicLostFrames, uint32* AcyclicFrames, uint32* AcyclicLostFrames);
DMC_API short __stdcall nmc_clear_master_diagnosis_info(WORD CardNo, WORD Channel);
DMC_API short __stdcall nmc_set_slave_port_state(WORD CardNo, WORD SlaveId, WORD Port, WORD State);
DMC_API short __stdcall nmc_get_slave_port_state(WORD CardNo, WORD SlaveId, WORD* PortState);
DMC_API short __stdcall nmc_get_exclude_slave_list(WORD CardNo, WORD* SlaveNum, WORD* SlaveList);
DMC_API short __stdcall nmc_hot_connect_accept_topo_change(WORD CardNo, WORD Channel);
DMC_API short __stdcall nmc_hot_connect_get_mode(WORD CardNo, uint32* Mode, WORD* BoardClose, WORD* ErrStop);
DMC_API short __stdcall nmc_hot_connect_get_group_num(WORD CardNo, uint32* GroupNum);
DMC_API short __stdcall nmc_hot_connect_get_group_present(WORD CardNo, uint32 GroupIndex,WORD* Present);
DMC_API short __stdcall nmc_hot_connect_get_all_group_present(WORD CardNo, uint32* GroupNum, uint32* PresentList);
DMC_API short __stdcall nmc_hot_connect_get_slave_group(WORD CardNo, WORD NodeId, uint32* GroupIndex);
DMC_API short __stdcall nmc_hot_connect_get_group_slaves(WORD CardNo, uint32 GroupIndex, WORD* SlaveNum, WORD* SlaveList);
DMC_API short __stdcall nmc_foe_download_file(WORD CardNo, uint32 SlaveId, uint32 Password, uint32 Timeout, const char* pFileName, const char* pFileNameControl);
DMC_API short __stdcall nmc_foe_upload_file(WORD CardNo, uint32 SlaveId, uint32 Password, uint32 Timeout, const char* pFileName, const char* pFileNameControl);

//看门狗功能扩展：支持局部输出口复位设置
DMC_API short __stdcall dmc_set_watchdog_write_outport_mask(WORD CardNo, uint32 PortNum, uint32 PortValue);
DMC_API short __stdcall dmc_get_watchdog_write_outport_mask(WORD CardNo, uint32 PortNum, uint32 * PortValue);
DMC_API short __stdcall dmc_set_watchdog_write_outport_mask_ex(WORD CardNo, uint32 PortNum, uint32 PortValue);
DMC_API short __stdcall dmc_get_watchdog_write_outport_mask_ex(WORD CardNo, uint32 PortNum,uint32 * PortValue);
DMC_API short __stdcall dmc_set_watchdog_outport_mask_enable(WORD CardNo, uint32 PortNum, uint32 Enable);
DMC_API short __stdcall dmc_get_watchdog_outport_mask_enable(WORD CardNo, uint32 PortNum, uint32* Enable);

//IO计数功能扩展：支持按照IO节点号进行设置计数
DMC_API short __stdcall nmc_set_io_count_mode(WORD CardNo, WORD PortNum, WORD NodeID, WORD BitNo, WORD Mode , WORD FilterTime);
DMC_API short __stdcall nmc_get_io_count_mode(WORD CardNo, WORD PortNum, WORD NodeID, WORD BitNo, WORD* Mode , WORD* FilterTime);
DMC_API short __stdcall nmc_set_io_count_value(WORD CardNo, WORD PortNum, WORD NodeID, WORD BitNo, WORD  CountValue);
DMC_API short __stdcall nmc_get_io_count_value(WORD CardNo, WORD PortNum, WORD NodeID, WORD BitNo, WORD* CountValue);

DMC_API short __stdcall nmc_get_exclude_slave_enable (WORD CardNo, WORD* Enable);

DMC_API short __stdcall dmc_m_add_line_moveseg_data_multi(WORD CardNo, WORD group, WORD AxisNum, WORD* AxisList, double* Target_pos, long mark);

DMC_API short __stdcall dmc_cmd_buf_set_line_profile_ext(WORD CardNo, WORD group, WORD plan_mode, double start_vel, double max_vel, double stop_vel, double tacc, double tdec, double ja,double jd);
