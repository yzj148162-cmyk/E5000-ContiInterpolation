...
本文档用于规划 `cdpr_control` 从当前“单轴速度模式位置闭环测试程序”扩展为“8绳6自由度绳驱并联机器人力交互控制程序”的软件架构。
...
本方案采用 E3000 控制卡。底层已完成卡型无关命名：`LeadshineMotionCard` 封装雷赛 SDK，`MotionCardHardwareInterface` 提供独占硬件线程边界，`MotionControlWorker` 承担控制状态机。连续插补仍作为历史测试模块保留，但不再代表程序的总体用途。
...
- 轴号、绳索号、由初始位姿逆解得到的初始绳长、电机零位、传动方向之间的映射逻辑。
...
- `MotionControlWorker` 运行在控制逻辑线程，`MotionCardHardwareInterface` 内部硬件线程独占 LTDMC；
...
- 底层类名、应用名和窗口标题已改为卡型无关的 CDPR 控制命名；雷赛函数调用顺序和线程边界保持不变；
...
#### 4.4.3 动平台刚体参数与Newmark设置

当前样机只维护一套经确认的动平台刚体参数：真实动平台、六维力传感器、工装和固定负载的质量、质心与惯量。该参数同时用于重力承载计算和Newmark纯惯性运动求解，不再额外配置一套“虚拟刚体质量/惯量”，避免相同样机参数重复维护和含义混淆。

这里的“纯惯性”是指动力学方程仍需要质量矩阵和惯量矩阵，但不包含回中刚度、虚拟阻尼、人为衰减或重力项。若未来确实要模拟与样机不同的目标航天器，再单独引入等效目标刚体配置，不能在当前版本预设一份重复参数。
...
6. 旋转矩阵是否正交且行列式接近1；
7. 动平台质量、卷筒直径和相对初始位置最大圈数是否为有效正数；
8. 动平台惯量是否对称正定，并满足基本刚体惯量关系；
9. 轴位置、速度、加速度上下限是否有序；
10. 是否能由初始位姿逆运动学得到8个有限正绳长；
11. 将逆解初始绳长送入正运动学后是否收敛并回到配置初始位姿；
...
- 预紧只在机器人启动前建立；
- 运行期张力分配只承担真实动平台及其固定载荷的重力，目标力矩为零；Newmark纯惯性期望运动不额外引入重力、刚度或阻尼；
...
2. `MotionCardHardwareInterface::configureTrace()` 已能按传入轴列表配置每轴 type 3/4/5/6，并在内部以动态轴列表解析采样值；
3. 但对外高频帧 `TraceTelemetryFrame` 仍是长度为2的固定数组；
4. `MotionCardHardwareInterface` 生成 `TraceTelemetryFrame` 时明确使用 `min(2, traceAxes.size())`，因此记录、延迟标定和速度闭环消费链路最多得到2轴；
...
还必须注意：当前 LTDMC Trace 读取链没有为每帧提供绝对硬件时间戳。程序使用
“本地连续读到的帧序号×固定采样周期”重建 `traceTimeUs`。批量到达本身不破坏
帧间时间关系，但卡侧缓冲耗尽、本地队列丢弃或非整帧数据都会使时间映射失去
可信依据。机器人模式必须把卡侧 `valid/free`、本地丢弃计数和时间轴可信标志
纳入运行帧；时间轴不可信时不得继续做8轴时间对齐控制。
...
- 速度命令产点使用独立控制时钟，不等待Trace逐帧准时到达；
- 每个期望状态进入带逻辑时刻/序号的短历史队列；
- 新Trace帧到达后，以该帧逻辑时刻查询同一时刻的期望轴位置和速度，更新PID反馈修正；同一反馈帧不得重复积分；
- 运行期最大跟随误差使用 `theta_d(t-tau)-theta_a(t)`，排除已标定固定延迟，避免正常高速运动产生 `|v|tau` 误报警；
...
- `MainWindow` 创建独立的 `MotionControlWorker` 控制逻辑线程；
- `MotionCardHardwareInterface` 内部再维护唯一硬件线程和硬件后端；
- LTDMC调用通过阻塞或排队方式进入硬件线程执行；
- `TelemetryRecorder` 内部已有独立写盘线程；
- UI以约20 Hz更新状态和测试曲线。

机器人框架必须复用这一硬件访问边界，不能再创建第二个能够直接调用LTDMC的“机器人硬件线程”。新增的是机器人控制协调线程、传感器线程和正运动学线程；机器人命令最终仍通过现有硬件接口提交。
...
设机器人控制周期为 `T_c`，控制产点和Trace反馈采用两条独立但可映射的时间链：

1. 传感器线程发布最新力旋量帧 `Wrench(k)`；
2. 控制线程按自身固定逻辑周期检查力样本新鲜度；
3. 对传感器读数执行零偏、重力、坐标和参考点变换；
4. `NewmarkBetaStepper` 根据当前ZOH力样本推进得到 `PlatformDesired(k+1)`；
5. 检查期望位姿、速度、加速度和欧拉角奇异性；
6. 逆运动学得到 `CableDesired(k+1)`；
7. `CableMotorMapper` 生成8轴期望位置和当前速度前馈，并把期望状态写入短历史队列；
8. 若本周期取得新的完整8轴Trace帧，则按该帧逻辑时刻查找同帧期望状态，更新8轴P/I/D反馈修正；否则保持上一反馈修正且不重复积分；
9. 当前速度前馈与最近反馈修正合成 `AxisCommand(k+1)`；
10. 执行8轴公共速度/加速度缩放和最终安全限幅；
11. 把带截止时间的整帧命令提交给硬件线程；
12. 硬件线程顺序下发8个 `dmc_change_speed_unit`；
13. 新Trace帧中的实际绳长异步送往正运动学线程；
14. 全部结构化数据异步送往记录线程。
...
Newmark产点和速度前馈使用控制逻辑周期；PID积分只在新Trace帧到达时使用相邻
反馈帧的逻辑间隔。两种 `dt` 都必须限制允许范围。若Trace时间轴不可信，或出现
异常长周期，不能按异常 `dt` 一次积分出大位移，应冻结反馈修正并进入降速或停机。
...
- 保存为新配置；
- 显示配置ID、文件哈希和校验状态；
- 分组显示几何、动平台刚体、Newmark设置、传感器、绳索轴映射和安全边界；
- 运行期间只读；
...
第一阶段无需立即移动所有现有文件。可以先新增这些模块，通过 `RobotHardwareAdapter` 调用现有 `MotionCardHardwareInterface`；底层类名、应用名和窗口标题清理已经完成。
...
3. 确认动平台、传感器、工装和固定载荷总成的质量、质心与惯量；当前Newmark纯惯性求解复用这套参数；
4. 确认是否启用滑轮修正；若启用，补齐8组滑轮中心、半径、轴向和实际切点几何；
5. 确认六维力传感器型号、通讯方式、采样率和硬件时间戳能力；
6. 标定8个轴与8根绳索的映射、收放绳方向和电机零位；初始绳长由初始末端位姿逆解，不作为独立配置项；
7. 确认卷筒直径160 mm及相对启动位置双向6.5圈限制，并评估有效直径是否随缠绕层数变化；
