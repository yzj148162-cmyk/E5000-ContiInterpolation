...
以下信息不阻塞阶段0骨架搭建，可以先在JSON中保留明显的无效占位值；但配置校验必须阻止带占位值的配置进入实机准备状态：

1. 8个机架锚点 `A_i` 和8个平台吊点 `b_i` 的最终坐标；
2. 8组滑轮中心、半径、轴向和实际出绳几何；
3. 世界、平台、传感器三个坐标系的方向、原点和安装变换；
4. 六维力传感器型号、通讯方式、采样率和数据时间戳能力；
5. 动平台、传感器、工装和负载的真实质量、真实质心和真实惯量；
6. 被模拟自由漂浮刚体的等效质量、等效质心和等效惯量；
7. 8个轴与8根绳索的映射、正方向、初始绳长和电机零位；
8. 卷筒有效半径及其是否随缠绕层数变化；
9. 机构工作空间、绳长范围、轴位置/速度/加速度安全限值；
10. 是否具备绳力传感器或其他松绳检测手段，以及预紧建立流程；
11. 8轴 `dmc_change_speed_unit` 实测批次耗时后确定的控制周期；
12. E3000可同时配置的Trace对象数量、单帧字节数和稳定采样周期。
