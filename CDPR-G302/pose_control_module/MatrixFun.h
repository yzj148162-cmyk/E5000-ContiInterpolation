/*
 * 文件总览：
 * - MatrixFun 提供项目内通用数学工具，包括 vector/Eigen 转换、矩阵运算、姿态转换、样条、伪逆/SVD 和带滑轮绳长计算。
 * - 该命名空间被轨迹、仿真、动力学、Nokov 位姿和安全监控等模块共享，是低层工具库而不是业务流程入口。
 * - 模板函数必须留在头文件中，非模板函数实现在 MatrixFun.cpp。
 */
...
namespace MatrixFun {
    ...
    **/
    ...
    // 根据 DH 参数序列计算每个关节坐标系到全局/基座的变换矩阵。
