/*
 * 文件总览：
 * - SingleMotorTrajectoryWorker 的实现文件，负责单轴轨迹生成、定时采样、命令位置/反馈位置读取和状态上报。
 * - buildTrajectory 只生成期望曲线，loop 才执行实时读取和 UI 数据刷新。
 */
...
QString trajectoryMotorDisplayName(int axisIndex)
{
...
void SingleMotorTrajectoryWorker::startTrajectory(const SingleMotorTrajectoryWorker::Command& command)
{
    ...
    if(command.axisIndex < 0){
        finishActiveTrajectory(QStringLiteral("电机编号无效"), "error");
        return;
    }
    ...
    const std::vector<double> motorVelMax = {effectiveCommand.maxVelocity};
    hardwareInterface->motorPosTraj(motorIndex, motorPosTraj, motorVelTraj, motorVelMax, timeStamp);
    ...
    emit displayInfoSignal(QStringLiteral("单电机轨迹精度测试：%1轨迹已下发，点数%2，时长%3 s")
                           .arg(trajectoryMotorDisplayName(effectiveCommand.axisIndex))
                           .arg(position.size())
