...
/*
 * 文件总览：
 * - NokovMinimalClient 的实现文件，把 Nokov SDK 的 C 风格回调转换为类成员处理，并缓存最新帧数据。
 * - ProcessFrameData 会将标记点、未命名点和刚体数据整理成 Qt 容器，同时计算简单速度/加速度字段。
 * - 外部回调若已注册，会在帧处理后被调用，用于调试或实时数据转发。
 */

// 构造函数
NokovMinimalClient::NokovMinimalClient() 
    : _client(nullptr), _initialized(false), _frameDataEnabled(false), _lastTimestamp(0)
{}
...
void NokovMinimalClient::SetFrameDataEnabled(bool enabled)
{
    ...
    if (!enabled) {
        ...
        _unnamedMarkers.clear();
        _lastMarkers.clear();
        _prevLastMarkers.clear();
        _lastUnnamedMarkers.clear();
        _prevLastUnnamedMarkers.clear();
...
void NokovMinimalClient::ProcessFrameData(const sFrameOfMocapData* data)
{
    ...
    for (int i = 0; i < data->nRigidBodies; ++i) {
        ...
        rigidBody.qz = rb.qz;
        rigidBody.qw = rb.qw;
        rigidBody.meanError = rb.MeanError;
        rigidBody.params = rb.params;

        rigidBody.markers.clear();
        if (rb.Markers && rb.nMarkers > 0) {
            rigidBody.markers.reserve(rb.nMarkers);
            for (int j = 0; j < rb.nMarkers; ++j) {
                MarkerPoint marker;
                marker.id = rb.MarkerIDs ? rb.MarkerIDs[j] : j;
                marker.x = rb.Markers[j][0];
                marker.y = rb.Markers[j][1];
                marker.z = rb.Markers[j][2];
                marker.xVel = marker.yVel = marker.zVel = 0.0f;
                marker.xAccel = marker.yAccel = marker.zAccel = 0.0f;
