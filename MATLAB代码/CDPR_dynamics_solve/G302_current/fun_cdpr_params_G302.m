function para_cdpr = fun_cdpr_params_G302
...
% 悬吊质量参数
mass_ee = 30; %mass of the EE(kg)
Ixx_ee = 0.1016*35/15; %inertial of the EE(kg*m2)
Iyy_ee = 0.165*35/15;
Izz_ee = 0.165*35/15;
...
% 以质心为原点建立local frame，将坐标从几何中心偏移至质心
CoM_x = 0;
CoM_y = 0;
CoM_z = -0.35*2/3;%-0.35*2/3;%-0.1
...
% 六维力传感器安装关系
sensor_origin_geom_e = [0;0;0;1];%[0; 0; -232.5e-3; 1]; %传感器原点位于几何中心正下方232.5mm处
sensor_origin_e = sensor_origin_geom_e - [CoM_x; CoM_y; CoM_z; 0];
sensor_R_ES = diag([1, -1, -1]); %传感器x轴同向，y/z轴相对动平台local frame反向
sensor_sign = 1; %1表示传感器数据为“环境作用在动平台上”的力，若符号相反改为-1

ao_e = [0; 0; 0; 1]; %the origin of the local frame

para_cdpr.mass_ee = mass_ee;
para_cdpr.M0_ee = diag([mass_ee, mass_ee, mass_ee, Ixx_ee, Iyy_ee, Izz_ee]);
para_cdpr.base_g = base_g;
para_cdpr.attach_e = attach_e;
para_cdpr.ao_e = ao_e;
para_cdpr.CoM_e = [CoM_x; CoM_y; CoM_z; 1];
para_cdpr.sensor_origin_geom_e = sensor_origin_geom_e;
para_cdpr.sensor_origin_e = sensor_origin_e;
para_cdpr.sensor_R_ES = sensor_R_ES;
para_cdpr.sensor_sign = sensor_sign;
