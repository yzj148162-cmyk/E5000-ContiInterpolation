function plot_trajectory_xiaoyuan(result_trj_ee, base_g, result_trj_cable,ideal_a_g)
%% 读取绘图参数
% 读取末端轨迹
...
%% 初始化绘图
% 空间轨迹
space_plot = plot_init_spaceTrj_xiaoyuan(ax_space, pose_ee, base_g,ideal_a_g);
...
% set(ax_space, 'XLim', [-4.5 4.5], 'YLim', [-4 4], 'ZLim', [-0 4]);
set(ax_space, 'XLim', [-1.5 1.5], 'YLim', [-1.5 1.5], 'ZLim', [-0 3]);
% 平面轨迹
...
for i = 1:size(t_vec,2)
    % 更新空间轨迹
    plot_update_spaceTrj_xiaoyua(space_plot, pose_ee(:,i), base_g, ideal_a_g(:,i),pose_ee(:,1:i));
