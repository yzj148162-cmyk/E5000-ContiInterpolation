...
void CDMCd1Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_ChangePos, m_dChangePos);
...
void CDMCd1Dlg::OnTimer(UINT nIDEvent) 
{
	...
	string.Format("%.2f", dPos );
	GetDlgItem( IDC_EDIT_CurPos )->SetWindowText( string );

	// 读取轴编码器脉冲位置值
	double dEncValue = 0;
	nRtn = dmc_get_encoder_unit(m_wCard,m_wAxis, &dEncValue);
	string.Format("%.2f", dEncValue );
	GetDlgItem( IDC_EDIT_CurEncPos )->SetWindowText( string );   

	//获取当前轴速度
	double dSpeed = 0;
	nRtn = dmc_read_current_speed_unit( m_wCard,m_wAxis,&dSpeed );    
	string.Format("%.2f", dSpeed );
	GetDlgItem( IDC_EDIT_CurSpeed)->SetWindowText( string );
...
void CDMCd1Dlg::OnButtonStartMove() 
{
	...
	//点动(位置模式)
	dmc_pmove_unit(m_wCard, m_wAxis, m_dDistance, 0);	
	UpdateData(false);
}
...
void CDMCd1Dlg::OnButtonStop() 
{
	// TODO: Add your control notification handler code here
	m_wAxis = m_nCombox_AxisNum.GetCurSel();
	dmc_stop(m_wCard,m_wAxis,0);          //减速停止
}
...
void CDMCd1Dlg::OnButtonChangePos() 
{
	// TODO: Add your control notification handler code here	
	m_wAxis = m_nCombox_AxisNum.GetCurSel();
	dmc_reset_target_position_unit(m_wCard, m_wAxis, m_dChangePos);
}
