...
//IO精确停止
DMC_API short __stdcall dmc_set_io_exactstop(WORD CardNo,WORD axis, WORD ioNum,WORD *ioList,WORD enable,WORD valid_logic,WORD action); 
...
功能：检查脉冲计数值和编码器反馈值之间差值是否超过报警阀值
	...
	1：差值大于等于报警阀值
*/
DMC_API short __stdcall dmc_check_pulse_encoder_count_error(WORD CardNo,WORD axis);
...
*/
DMC_API short __stdcall dmc_update_target_position_extern(WORD CardNo, WORD axis, double mid_pos, double aim_pos, WORD posi_mode);
...
/*****************************总线相关函数**********************************/
//回零运动
DMC_API short __stdcall nmc_set_home_profile(WORD CardNo ,WORD axis,WORD home_mode,double High_Vel, double Low_Vel,double Tacc,double Tdec ,double offsetpos);//设置回零参数，合并函数
DMC_API short __stdcall nmc_get_home_profile(WORD CardNo ,WORD axis,WORD* home_mode,double* High_Vel, double* Low_Vel,double* Tacc,double* Tdec ,double* offsetpos);
...
//扩展PDO
DMC_API short __stdcall nmc_write_rxpdo_extra(WORD CardNo,WORD PortNum,WORD address,WORD DataLen,DWORD Value);
DMC_API short __stdcall nmc_read_rxpdo_extra(WORD CardNo,WORD PortNum,WORD address,WORD DataLen,DWORD* Value);
DMC_API short __stdcall nmc_read_txpdo_extra(WORD CardNo,WORD PortNum,WORD address,WORD DataLen,DWORD* Value);
...
DMC_API short __stdcall nmc_get_cycleratio(WORD CardNo,WORD PortNum,WORD* CycleRatio);
#ifdef __cplusplus
}
#endif
