...
namespace 定长运动
{
    public partial class Form1 : Form
    {
        ...
        private void btn_Start_Click(object sender, EventArgs e)
        {
            ...
            dDist = decimal.ToDouble(nud_Dist.Value);
            usPosiMode = 0;
            ...
            LTDMC.dmc_pmove_unit(usCardId, usAxis, dDist, usPosiMode);                               //定长运动
        }
        ...
        {
            ushort usAxis = GetAxis();                             //轴号
            double dEquiv= decimal.ToDouble(nud_PulseEquiv.Value); //脉冲当量          
        ...
        private void btn_ChangeVel_Click(object sender, EventArgs e)
        {
            ...
            dNewVel = decimal.ToDouble(nud_NewVel.Value);
            dTaccDec = decimal.ToDouble(nud_TaccDec.Value);
            LTDMC.dmc_change_speed_unit(usCardId, usAxis, dNewVel, dTaccDec); //在线变速
 
        }

        private void btn_ChangePos_Click(object sender, EventArgs e)
        {
            ushort usAxis = GetAxis();  //轴号
            double dNewPos;             // 新的目标位置
            dNewPos = decimal.ToDouble(nud_NewPos.Value);
            LTDMC.dmc_reset_target_position_unit(usCardId, usAxis, dNewPos);//在线变位
        }
