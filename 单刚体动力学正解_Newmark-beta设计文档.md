...
CDPR状态推进统一采用本文的Newmark-beta方法。实时控制在 \(t_k\) 使用当前
已经取得的力样本 \(W_k\)，并在本模型周期内保持：

\[
W(t)=W_k,\qquad t\in[t_k,t_{k+1})
\]

Newmark求得的 \(\ddot q_{k+1}\) 是动力学方程中的下一状态加速度，不代表使用了
未来力样本。

CDPR正式控制还遵循：

- 当前模拟力最小实时闭环和离线连续验证统一使用5 ms步长，`G302.m` 的50 ms
  只属于离线算例；后续缩短周期必须先验证完整周期耗时裕量；
...
当前C++工程已完成 `CdprDynamics` Newmark-beta单步模块及静止、恒力离线
测试，并接入两条互相隔离的5 ms运行链：一条是完全不访问控制卡的单步/连续
离线验证，另一条是模拟六维力驱动8轴速度闭环的实机链。两条链只共享算法和
不可变参数，各自持有独立状态；真实F/T Trace和Nokov位姿重建仍待接通。
