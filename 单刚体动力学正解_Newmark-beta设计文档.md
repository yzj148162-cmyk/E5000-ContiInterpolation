...
### 1.1 在CDPR实时力交互中的实现边界

`MATLAB代码/CDPR_dynamics_solve/G302_current` 当前采用四元数RK4，并非
Newmark-beta。后续CDPR软件只参考其中的自由刚体方程、坐标关系和六维力转换，
状态推进统一采用本文的Newmark-beta方法。

必须区分“状态积分方法”和“离散输入保持方式”：

- Newmark-beta是二阶动力学状态积分方法；
- ZOH/FOH描述力传感器离散样本在两个采样时刻之间如何解释，不属于Newmark公式。

实时控制在 \(t_k\) 时只能取得当前力样本 \(W_k\)，无法无延迟取得未来样本
\(W_{k+1}\)。第一版因此采用因果输入：

\[
W(t)=W_k,\qquad t\in[t_k,t_{k+1})
\]

即当前力样本在本控制步内保持不变。Newmark求得的 \(\ddot q_{k+1}\) 是动力学
方程中的下一状态加速度，不等于提前取得了 \(W_{k+1}\)。

若使用 \(W_k\) 和 \(W_{k+1}\) 对区间做FOH，必须等下一帧到达后再计算上一段，
这会增加至少一个采样周期的控制延迟。除非后续明确接受该延迟，否则实时执行不采用
这种无预测的延迟FOH。

CDPR正式控制还遵循：

- 控制周期暂按1 ms设计，`G302.m` 的50 ms只属于离线算例；
- 期望刚体采用无回中刚度、无虚拟阻尼、无重力项的纯惯性模型；
- 当前版本直接使用已确认的动平台总成质量和惯量，不再重复配置“虚拟刚体质量/惯量”；
- 真实平台重力由实际绳索张力承担，不写入Newmark期望运动方程；
- 外力撤除后，期望刚体保持已有速度和角速度自由漂移。
